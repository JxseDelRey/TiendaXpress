/**
 * Script de Seed — TiendaXpress
 * Puebla la base de datos con categorías, productos y usuario admin de prueba.
...
const ProductSchema = new mongoose.Schema({
  ...
  category: mongoose.Schema.Types.ObjectId, stock: Number, lowStockAlert: Number,
...
async function seed() {
  ...
  // Crear categorías
  const createdCats = await CategoryModel.insertMany(
  ...
  createdCats.forEach((c: any) => { catMap[c.name] = c._id; });
  ...
  const createdProducts = await ProductModel.insertMany(
    products.map(p => ({ ...p, isActive: true, soldCount: Math.floor(Math.random() * 50) }))
  );
